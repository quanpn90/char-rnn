...
cmd:text('Options')
-- required:
cmd:option('-model', 'cv/lm_lstm_epoch50.00_1.3898.t7', 'model checkpoint to use for sampling')
-- optional parameters
...
if string.len(seed_text) > 0 then
    ...
    for c in seed_text:gmatch'.' do
        ...
        prediction = protos.rnn:forward(prev_char)
...
for i=1, opt.length do
    ...
    if opt.sample == 0 then
		...
		prev_char:resize(1, 1)
		prev_char = prev_char:cuda()
    --~ print(prev_char)
    -- forward the rnn for next character
		prediction = net:forward(prev_char)

    io.write(ivocab[prev_char[1][1]])
end
io.write('\n') io.flush()
