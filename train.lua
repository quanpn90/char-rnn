...
if string.len(opt.init_from) > 0 then
    ...
    net = nn.Sequential()
		net:add(nn.LookupTable(vocab_size, opt.rnn_size))
		
		
		local rnn = cudnn.LSTM(opt.rnn_size, opt.rnn_size, opt.num_layers, false, opt.dropout, true)

		net:add(rnn)

		local linear = nn.Linear(opt.rnn_size, vocab_size)

		net:add(nn.Sequencer(linear))
		net:add(nn.Sequencer(cudnn.LogSoftMax()))
...
local rnn = net.modules[2] -- a pointer to the rnn
...
trainState.hiddenOutput = torch.CudaTensor()
trainState.hiddenCell = torch.CudaTensor()
...
function eval_split(split_index, max_batches)
		...
		trainState.hiddenOutput = rnn.hiddenOutput:clone()
		trainState.cellOutput = rnn.cellOutput:clone()
		
		rnn.hiddenOutput:zero()
		rnn.cellOutput:zero()
    ...
    -- back to the current train state again
    rnn.hiddenOutput:resize(trainState.hiddenOutput:size()):copy(trainState.hiddenOutput)
    rnn.cellOutput:resize(trainState.cellOutput:size()):copy(trainState.cellOutput)
    net:training()
...
function feval(x)
    ...
    local loss = criterion:forward(net_output, y)
    
    loss = loss / opt.seq_length
    
    local dloss = criterion:backward(net_output, y)
    
    net:backward(x, dloss)
